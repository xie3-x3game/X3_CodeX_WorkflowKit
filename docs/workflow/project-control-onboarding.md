...
```text
先用最小可行结构推进，后续根据真实使用复盘扩展。
```

含义：

- 新项目先建立必要的总控、索引和少数高频窗口，不提前展开完整组织结构。
- 新专项先挂在父项目下运行；只有投入度、复杂度或长期价值上升后，才升级为独立项目。
- 新人员、AI 角色、沟通记录和权限模型先记录最小字段，后续随着协作真实发生再扩展。
- 新流程先用草稿、模板、清单或手动步骤验证；重复出现且风险明确后，再升级为规则、脚本或自动化。
- 项目总控定期或在新内容加入时复盘结构，决定是否合并、拆分、升级、迁移、休眠或归档。
